...
function findTitleFromJsonLd(items) {
  ...
  for (const item of items) {
    ...
    if (item.name && !/BreadcrumbList|WebSite/i.test(type || "")) {
      return cleanTitle(item.name);
    }
  }
...
function fallbackTitleFromUrl(target, site) {
  try {
    ...
    if (site === "rakuten" && parts.length >= 2) {
      const shop = parts[0] || "";
      const itemCode = parts[1] || "";
      return `楽天商品 / ${shop} / ${itemCode}`;
    }

    if (site === "yahoo" && parts.length >= 1) {
      return `Yahoo商品 / ${parts[parts.length - 1]}`;
    }

    if (site === "amazon" && parts.length >= 1) {
      const asinMatch = target.pathname.match(/\/dp\/([A-Z0-9]{10})|\/gp\/product\/([A-Z0-9]{10})/i);
      const asin = asinMatch?.[1] || asinMatch?.[2];

      if (asin) return `Amazon商品 / ${asin}`;

      return `Amazon商品 / ${parts[parts.length - 1]}`;
    }
...
async function fetchDirect(target) {
  ...
  if (!response.ok) {
    throw new Error(`쇼핑몰 응답 오류: HTTP ${response.status}`);
  }
...
async function fetchViaJina(target) {
  ...
  if (!response.ok) {
    throw new Error(`Reader 응답 오류: HTTP ${response.status}`);
  }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const finalTitle = product.title || fallbackTitleFromUrl(target, site);
